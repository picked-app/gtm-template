___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Picked",
  "categories": ["SALES", "ANALYTICS", "EXPERIMENTATION"],
  "brand": {
    "id": "brand_dummy",
    "displayName": "Picked",
    "thumbnail": "data:image/png;base64,\u003d"
  },
  "description": "The Picked template helps you track ecommerce, form, modal, and custom events and surface insights directly in Picked.",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[
  {
    "type": "TEXT",
    "name": "tagId",
    "displayName": "Tag ID",
    "simpleValueType": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "help": "This value is provided in the Picked Dashboard."
  },
  {
    "type": "SELECT",
    "name": "eventType",
    "displayName": "Event",
    "macrosInSelect": true,
    "selectItems": [
      {
        "value": "cart_add",
        "displayValue": "Cart add (cart_add)"
      },
      {
        "value": "cart_remove",
        "displayValue": "Cart remove (cart_remove)"
      },
      {
        "value": "checkout_complete",
        "displayValue": "Checkout complete (checkout_complete)"
      },
      {
        "value": "checkout_start",
        "displayValue": "Checkout start (checkout_start)"
      },
      {
        "value": "custom",
        "displayValue": "Custom (custom)"
      },
      {
        "value": "field_validation_error",
        "displayValue": "Field validation error (field_validation_error)"
      },
      {
        "value": "form_submit",
        "displayValue": "Form submit (form_submit)"
      },
      {
        "value": "form_submit_error",
        "displayValue": "Form submit error (form_submit_error)"
      },
      {
        "value": "waitlist_join",
        "displayValue": "Join waitlist (waitlist_join)"
      },
      {
        "value": "lead_capture",
        "displayValue": "Lead capture (lead_capture)"
      },
      {
        "value": "modal_close",
        "displayValue": "Modal close (modal_close)"
      },
      {
        "value": "modal_open",
        "displayValue": "Modal open (modal_open)"
      },
      {
        "value": "order_create",
        "displayValue": "Order create (order_create)"
      },
      {
        "value": "page_view",
        "displayValue": "Page view (page_view)"
      },
      {
        "value": "product_list_view",
        "displayValue": "Product list view (product_list_view)"
      },
      {
        "value": "product_select",
        "displayValue": "Product select (product_select)"
      },
      {
        "value": "product_view_details",
        "displayValue": "Product view details (product_view_details)"
      },
      {
        "value": "recommendation_present",
        "displayValue": "Recommendation present (recommendation_present)"
      },
      {
        "value": "search_results_list_view",
        "displayValue": "Search results list view (search_results_list_view)"
      },
      {
        "value": "wishlist_add",
        "displayValue": "Wishlist add (wishlist_add)"
      }
    ],
    "simpleValueType": true,
    "alwaysInSummary": true,
    "valueValidators": [
      {
        "type": "NON_EMPTY"
      }
    ],
    "help": "Related Picked event name"
  },
  {
    "type": "GROUP",
    "name": "CustomEvent",
    "displayName": "Custom Event",
    "groupStyle": "NO_ZIPPY",
    "enablingConditions": [
      {
        "paramName": "eventType",
        "paramValue": "custom",
        "type": "EQUALS"
      }
    ],
    "subParams": [
      {
        "type": "TEXT",
        "name": "customEventName",
        "displayName": "Custom Event Name",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          },
          {
            "type": "REGEX",
            "args": ["^[a-z0-9_-]+$"],
            "errorMessage": "Custom event name must be lowercase alphanumeric with underscores and hyphens only."
          }
        ],
        "help": "The name of the custom event to track (e.g. \"signup_complete\"). Must be lowercase alphanumeric with underscores and hyphens only."
      },
      {
        "type": "CHECKBOX",
        "name": "customIncludeForm",
        "checkboxText": "Include Form data",
        "simpleValueType": true,
        "defaultValue": false
      },
      {
        "type": "CHECKBOX",
        "name": "customIncludeModal",
        "checkboxText": "Include Modal data",
        "simpleValueType": true,
        "defaultValue": false
      },
      {
        "type": "CHECKBOX",
        "name": "customIncludeOrder",
        "checkboxText": "Include Order data",
        "simpleValueType": true,
        "defaultValue": false
      },
      {
        "type": "CHECKBOX",
        "name": "customIncludeException",
        "checkboxText": "Include Exception data",
        "simpleValueType": true,
        "defaultValue": false
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Form",
    "displayName": "Form",
    "groupStyle": "NO_ZIPPY",
    "enablingConditions": [
      {
        "paramName": "customIncludeForm",
        "paramValue": true,
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "form_submit",
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "form_submit_error",
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "field_validation_error",
        "type": "EQUALS"
      }
    ],
    "subParams": [
      {
        "type": "TEXT",
        "name": "formId",
        "displayName": "Form ID",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "help": "The unique identifier of the form."
      },
      {
        "type": "TEXT",
        "name": "formTitle",
        "displayName": "Form Title",
        "simpleValueType": true,
        "help": "The display title of the form (optional)."
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Modal",
    "displayName": "Modal",
    "groupStyle": "NO_ZIPPY",
    "enablingConditions": [
      {
        "paramName": "customIncludeModal",
        "paramValue": true,
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "modal_close",
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "modal_open",
        "type": "EQUALS"
      }
    ],
    "subParams": [
      {
        "type": "TEXT",
        "name": "modalId",
        "displayName": "Modal ID",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "help": "The unique identifier of the modal."
      },
      {
        "type": "TEXT",
        "name": "modalTitle",
        "displayName": "Modal Title",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "help": "The display title of the modal."
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Order",
    "displayName": "Order",
    "groupStyle": "NO_ZIPPY",
    "enablingConditions": [
      {
        "paramName": "customIncludeOrder",
        "paramValue": true,
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "order_create",
        "type": "EQUALS"
      }
    ],
    "subParams": [
      {
        "type": "TEXT",
        "name": "orderId",
        "displayName": "Order ID",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "help": "The unique identifier of the order."
      },
      {
        "type": "TEXT",
        "name": "orderCustomerId",
        "displayName": "Customer ID",
        "simpleValueType": true,
        "help": "The customer ID associated with the order (optional)."
      },
      {
        "type": "TEXT",
        "name": "orderCheckoutToken",
        "displayName": "Checkout Token",
        "simpleValueType": true,
        "help": "The checkout token for the order (optional)."
      },
      {
        "type": "TEXT",
        "name": "orderCartToken",
        "displayName": "Cart Token",
        "simpleValueType": true,
        "help": "The cart token for the order (optional)."
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Exception",
    "displayName": "Exception",
    "groupStyle": "NO_ZIPPY",
    "enablingConditions": [
      {
        "paramName": "customIncludeException",
        "paramValue": true,
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "field_validation_error",
        "type": "EQUALS"
      },
      {
        "paramName": "eventType",
        "paramValue": "form_submit_error",
        "type": "EQUALS"
      }
    ],
    "subParams": [
      {
        "type": "TEXT",
        "name": "exceptionReason",
        "displayName": "Exception Reason",
        "simpleValueType": true,
        "valueValidators": [
          {
            "type": "NON_EMPTY"
          }
        ],
        "help": "The reason for the error or validation failure."
      }
    ]
  },
  {
    "type": "GROUP",
    "name": "Ecommerce",
    "displayName": "Ecommerce",
    "groupStyle": "NO_ZIPPY",
    "subParams": [
      {
        "type": "CHECKBOX",
        "name": "sendEcommerceData",
        "checkboxText": "Send ecommerce data",
        "simpleValueType": true,
        "defaultValue": true,
        "alwaysInSummary": true
      },
      {
        "type": "SELECT",
        "name": "ecommerceData",
        "displayName": "GA4 Enhanced Ecommerce Data",
        "macrosInSelect": true,
        "selectItems": [],
        "simpleValueType": true,
        "enablingConditions": [
          {
            "paramName": "sendEcommerceData",
            "paramValue": true,
            "type": "EQUALS"
          }
        ],
        "alwaysInSummary": false
      }
    ]
  }
]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// https://developers.google.com/tag-platform/tag-manager/templates/api

const log = require('logToConsole');
const copyFromWindow = require('copyFromWindow');
const templateStorage = require('templateStorage');
const callInWindow = require('callInWindow');
const injectScript = require('injectScript');
const setInWindow = require('setInWindow');

const LOG_PREFIX = 'picked_gtm: ';

const PICKED_GTM_URL = 'https://api.picked.app/sdk/picked-gtm.js';

log(LOG_PREFIX + 'data =', data);

if (!copyFromWindow('picked_gtm')) {
  log(LOG_PREFIX + '\'picked_gtm\' is not defined in \'window\'. Creating it.');
  setInWindow('picked_gtm', []);
}

callInWindow('picked_gtm.push', data);

const onFailure = (msg) => {
  log(LOG_PREFIX + 'error: ' + msg);
  return data.gtmOnFailure();
};

const onSuccess = () => {
  log(LOG_PREFIX + 'success');
  return data.gtmOnSuccess();
};


injectScript(PICKED_GTM_URL, onSuccess, onFailure, 'picked_gtm');


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "picked_gtm"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": false
                  }
                ]
              },
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "picked_gtm.push"
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": false
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://api.picked.app/sdk/picked-gtm.js"
              },
              {
                "type": 1,
                "string": "https://*.convincely.io/sdk/picked-gtm.js?x-vercel-protection-bypass\u003d*"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_template_storage",
        "versionId": "1"
      },
      "param": []
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 20/01/2025, 16:58:48

